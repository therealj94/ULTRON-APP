...
export const Arranque: React.FC<{ visible: boolean; estado: string }> = ({ visible, estado }) => {
  ...
  return (
    <div
      id="ultron-arranque"
      aria-hidden={!visible}
      className={`absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#232528] transition-opacity duration-700 ${
        visible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
      }`}
    >
      <div className="relative flex flex-col items-center gap-6 px-6">
        <img
          src="/marca/logo-aura-oscuro.png"
        ...
        />
        {perfil.demo && <div className="text-[12px] font-medium text-[#8A847C]">Demostración · no es producción</div>}
        <div className="flex flex-col items-center gap-3 transition-opacity duration-500" style={{ opacity: fase === 2 ? 1 : 0 }}>
          <div className="flex gap-2" aria-hidden="true">
            {[0, 1, 2].map((i) => (
              <span key={i} className="w-2.5 h-2.5 rounded-full bg-[#D6B56C] animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
            ))}
          </div>
          <div className="h-5 text-[14px] text-[#B9B2A8]">{estado}</div>
        </div>
      </div>
    </div>
  );
