...
export function normalizar(raw: string): string {
  return String(raw || '')
    ...
    .replace(/^(?:ultron|aura|au-ra|au ra)[,:\s]+/, '')
    .replace(/[,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
const REGLAS: Regla[] = [
  ...
  },
  {
    ...
    build: (m, _q, raw) => {
      const hecho = raw.replace(/^[\s¡¿"'«»]*(oye|hey|ey)?[,\s]*((?:ultron|aura|au-ra|au ra)[,:\s]+)?/i, '').replace(/^(recuerda|recordá|recorda|memoriza|memorizá)\s+que\s+/i, '').trim();
