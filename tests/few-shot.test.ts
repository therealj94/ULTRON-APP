...
describe('Fase 2 — few-shot de honestidad', () => {
  it('incluye los 5 ejemplos en tareas de código', () => {
    ...
    assert.ok(messages[0].content.indexOf(FEW_SHOT_HONESTO.slice(0, 20)) > messages[0].content.indexOf('Eres ULTRON'));
  ...
  it('NO inyecta few-shot en un saludo (no ensucia la voz)', () => {
    const { messages, meta } = construirMensajes({ personalidad: 'Eres ULTRON.', user: 'hola, ¿todo bien?' });
